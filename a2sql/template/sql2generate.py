...
USER_PROXY_NO_EXE: str = """# 你是用户代理,负责协调整个 NL2SQL 任务流程。

# 你的职责:
1. 接收用户的自然语言查询
2. 协调各个 Agent 之间的交互
3. 确保任务按照正确的流程执行: NL_Analyst(自然语言分析) → SQL_Generator(SQL生成器)
4. 当所有任务完成且结果已生成时,回复 TERMINATE
...
TASK_PROMPT_NO_EXE: str = """[用户初始 NL 查询]
目标: 开发并执行一个 NL2SQL 任务。
...
SQL_GENERATOR_NO_EXE: str = """{schema}
...
现在请根据 NL_Analyst 的分析结果生成 SQL 语句,完成后添加 TERMINATE。
"""
...
SQL2GENERATE_NO_EXE = {
    "user_proxy": USER_PROXY_NO_EXE,
    "nl_analyst": NL_ANALYST,
    "task_prompt": TASK_PROMPT_NO_EXE,
    "sql_generator": SQL_GENERATOR_NO_EXE,
    "sql_executor": SQL_EXECUTOR,
    "refiner": REFINER,
}