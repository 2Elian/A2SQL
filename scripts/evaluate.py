...
def evaluate_complex(table, gold, predict, mode='exact', single_equal=False):
    ...
    for k,v in scores.items():
        if 'acc' in v.keys() and 'rec' in v.keys():
            ...
            if scores[k]['acc'] * scores[k]['rec'] < 0.0001:
                scores[k]['f1'] = 0.0
            else:
    ...
    for k,v in scores_novalue.items():
        if 'acc' in v.keys() and 'rec' in v.keys():
            ...
            if scores_novalue[k]['acc'] * scores_novalue[k]['rec'] < 0.0001:
                scores_novalue[k]['f1'] = 0.0
            else:
                scores_novalue[k]['f1'] = 2 * scores_novalue[k]['acc'] * scores_novalue[k]['rec'] \
